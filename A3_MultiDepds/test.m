...
for k=1:4
    ...
    while i < samplesize
        ...
        x = 0; sum_W = 0; tau = 0; delta_tau = 0;mu_Y = mu_Y_0; % reset these parameters
    %% Step 2
        beta_s = normrnd(mu_beta,sigma_beta);
        while 1
            handle_lambda = @(t) (lambda_0 + gamma_a*x + gamma_b*sum_W);
            T_1 = 5/handle_lambda(tau); % let T_1 be 5 times of the current mean arrival time
            delta_tau = NHPP(handle_lambda,T_1,tau);
            tau = tau + delta_tau;
        %% Step 3
            x = x + (beta_s + gamma_d*sum_W)*delta_tau; % the updated degradation rate only influences the degradation process after the moment of the updating
            if x > D
                i = i+1;
                t_f(i,k) = (D-x)/(beta_s + gamma_d*sum_W) + tau; % also changed here because of the changement of "x"
                break;
            end
        %% Step 4
